...
(defn >sys-request [template-name] {:template-name template-name
...
(facts ">GenericTemplateFinder"
  (let [gf (>GenericTemplateFinder 'cludje.templatefind-test 'cludje.altnamespace)]
    ...
    (fact "finds template"
      (fact "with string"
        (find-template gf (>sys-request "edit-cog")) => "Edit")
      (fact "with keyword"
        (find-template gf (>sys-request :edit-cog)) => "Edit"))
    (fact "works with another namespace"
      (find-template gf (>sys-request :add-cog)) => "Altns add")
    (fact "Not finding returns nil"
      (find-template gf (>sys-request :foo-bar)) => nil)
    (fact "Throws if there's no model"
      (find-template gf (>sys-request :edit-foo)) => (throws-error))))
...
(facts ">SpecificTemplateFinder"
  (let [mf (>SpecificTemplateFinder 'cludje.templatefind-test 'cludje.altnamespace)]
    ...
    (fact "finds template"
      (fact "with string"
        (find-template mf (>sys-request "show-cog")) => "Show")
      (fact "with keyword"
        (find-template mf (>sys-request :show-cog)) => "Show"))
    (fact "works with another namespace"
      (find-template mf (>sys-request :inc-cog)) => "Altns inc cog")
    (fact "Not finding returns nil"
      (find-template mf (>sys-request :bar-foo)) => nil)
    (fact "Runs even if there's no model"
      (find-template mf (>sys-request :show-foo)) => "Foo")))
...
(facts ">CompositeTemplateFinder"
  (let [cf (>CompositeTemplateFinder 'cludje.templatefind-test 'cludje.altnamespace)]
    (fact "Finds specific template first"
      (find-template cf (>sys-request :edit-cog)) => "Altns edit cog")
    (fact "Find generic if there's no specific"
      (find-template cf (>sys-request :delete-cog)) => "Delete")
    (fact "Doesn't find generics in specific namespace"
      (find-template cf (>sys-request :show-cog)) => nil)
    ))
